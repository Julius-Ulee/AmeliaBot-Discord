		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setDescription(`${client.allEmojis.x} Please add a Bassboost-Gain between 0 and 20!`)
						],
					})
				}
				...
				if (bass_gain > 20 || bass_gain < 0) {
					return message.reply({
						embeds: [
							new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setDescription(`${client.allEmojis.x} The Bassboost Gain must be between 0 and 20!`)
						],
					})
				}
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setDescription(`${client.allEmojis.check_mark} Set a Bassboost to ${bass_gain}!`)]
				})
			} catch (e) {
