		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setDescription(`${client.allEmojis.x} Please add a Seek Number Position-Duration! \`@${client.user.username} seek <Duration_in_Sec>\``)
						],
					})
				}
				...
				if (seekNumber > newQueue.songs[0].duration || seekNumber < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`${client.allEmojis.x} The Seek Position must be between \`0\` and \`${newQueue.songs[0].duration}\`!`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setDescription(`⏺ Seeked to \`${seekNumber} Seconds\`!`)]
				})
			} catch (e) {
