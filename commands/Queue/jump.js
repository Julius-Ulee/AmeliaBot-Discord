			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setDescription(`${client.allEmojis.x} Please add a Position to jump to! \`@${client.user.username} jump <position>\``)
						],
					});
				}
				...
				if (Position > newQueue.songs.length - 1 || Position < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`${client.allEmojis.x} The Position must be between \`0\` and \`${newQueue.songs.length - 1}\`!`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setDescription(`${client.allEmojis.check_mark} Jumped to the \`${Position}th\` Song in the Queue!`)]
				})
			} catch (e) {
