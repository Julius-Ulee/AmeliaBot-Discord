			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setDescription(`${client.allEmojis.x} Please add a Volume! \`@${client.user.username} volume <Percentage>\``)
						],
					});
				}
				...
				if (volume > 150 || volume < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`${client.allEmojis.x} The Volume must be between \`0\` and \`150\`!`)
					],
