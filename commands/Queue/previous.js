		...
		try {
			...
			try {
				...
				if (!newQueue || !newQueue.previousSongs || newQueue.previousSongs.length == 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`${client.allEmojis.x} There are no previous Songs!`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setDescription(`▶️ Now Playing the Previous Played Track!`)]
				})
			} catch (e) {
