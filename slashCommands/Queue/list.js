		...
		try {
			...
			try {
				...
				if (!newQueue || !newQueue.songs || newQueue.songs.length == 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`Queue for ${guild.name}`).setURL(`https://discord.gg/VzUR95y`).setDescription(`${client.allEmojis.x} The queue is empty`).setFooter(`0 Songs in the Queue | 00:00 Total Length`, ee.footericon)
					],
					ephemeral: true
				})
				let embeds = [];
				...
				for (let i = 0; i < theSongs.length; i += 10) {
					...
					let j = i;
					const info = current.map((track) => `\n\`${j++}.\` [${String(track.name).replace(/\[/igu, "{").replace(/\]/igu, "}").substr(0, 60)}](${track.url}) - \`${track.formattedDuration} Requested by: ${track.user.tag}\``).join("\n")
					const embed = new MessageEmbed()
						.setColor(ee.color)
						.setDescription(`${info}`)
					if (i < 10) {
					  embed.setTitle(`Queue for ${guild.name}`)
            embed.setURL(`https://discord.gg/VzUR95y`)
						embed.setDescription(`__Now Playing:__\n${client.allEmojis.Music} [${theSongs[0].name.replace(/\[/igu, "{").replace(/\]/igu, "}")}](${theSongs[0].url})\n\n__Up Next:__${info}`)
					}
					embeds.push(embed);
					k += 10; //Raise k to 10
				}
				embeds[embeds.length - 1] = embeds[embeds.length - 1]
					.setFooter(`📑 Top ${theSongs.length > 50 ? 50 : theSongs.length}  \n${theSongs.length} Songs in the Queue | ${newQueue.formattedDuration} Total Length`, ee.footericon)
				let pages = []
